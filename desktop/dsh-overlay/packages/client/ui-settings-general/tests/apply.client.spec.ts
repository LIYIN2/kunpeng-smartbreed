...
function seatEntry(slots: SlotRegistry, name: typeof SEATS[number][0], component: typeof SEATS[number][1]) {
  return slots.entries(name).find(entry => entry.component === component)
}
...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    for (const [name, component] of SEATS) {
      expect(seatEntry(before.slots, name, component)?.component).toBe(component)
    }
    ...
    for (const [name, component] of SEATS) {
      expect(seatEntry(before.slots, name, component)?.locale).toBe('settings')
    }
    ...
    for (const [name, component] of SEATS) {
      expect(seatEntry(after.slots, name, component)?.component).toBe(component)
  ...
  it('registers the zh/en settings dictionaries and frees the seats on teardown', async () => {
    ...
    await fiber.await()
    expect(b.locale.bind('settings')('title')).toBe('鲲鹏设置')
  ...
  it('re-registers after an HMR collapse of the declaring chain (stale disposers must not block)', async () => {
    ...
    await Promise.resolve()
    for (const [name, component] of SEATS) {
      expect(seatEntry(b.slots, name, component)?.component).toBe(component)
    }
