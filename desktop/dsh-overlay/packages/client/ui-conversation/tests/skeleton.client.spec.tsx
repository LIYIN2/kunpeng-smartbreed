...
describe('Hero chrome', () => {
  it('renders the Kunpeng research identity through the hero locale seat', () => {
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} />)
    expect(view.getByText('GENOMIC SELECTION · RESEARCH OPERATIONS · OPEN AGENTS')).toBeTruthy()
    expect(view.getByText('What will you work on today?')).toBeTruthy()
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('hero phase: same textarea, hero chrome, no header, picker switches the workspace', () => {
    ...
    expect(b.view.getByText('今天，从哪项科研工作开始？')).toBeTruthy()
    expect(b.view.getByText('全基因组选育')).toBeTruthy()
