...
- v2.4.5（当前）：完善 GitHub 技能包一键导入、`/` 技能检索调用与技能库浏览管理；安装包预装当前技能库的 36 个技能，首次启动即可浏览和调用。修复运行中临时转向、会话消息重复/错位及上传路径混入文本的问题；优化自适应输入框与附件展示。Windows Release 同时提供已验收的安装器与完整预览包。
