...
- v2.4.6（当前）：修复附件删除无响应、附件发送后残留在下一条草稿及已删除附件被历史上传目录重新检索的问题；附件展示改为紧凑卡片。内置 DeepSeek Vision 网关，选择 `DeepSeek + Vision` 的 `DeepSeek-V4-Flash-Vision-Exp` 即可使用 DeepSeek 原生读图能力。Release 提供 Windows 安装器、Windows 完整预览包和 macOS Apple Silicon DMG；macOS DMG 已修复首次启动运行时 preset 与归档签名生成顺序。
