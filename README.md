...
v2.3.0 已把六个首页卡片改为真实工作台，而不是只填入几句提示词：
...
`control-plane/` 是可运行的管理后台，包含 `owner` / `admin` / `reviewer` / `member` 四级角色、多管理员、账号启停、临时密码重置、首次登录强制改密、知识提交与审核、信息反馈和审计日志。提交人不能审核自己的知识条目。
...
- v2.3.0（当前开发版）：六个可操作工作台、RNA-seq/WGCNA/GWAS 专项 skill、SCI 投稿门禁、期刊雷达、账号与权限分层、知识审核、信息反馈、Windows 原生 CI 构建。
...
## 目录结构
    ...
    kunpeng-smartbreed/
    ├── desktop/                     # Electron 桌面端、Harness UI 覆盖层与打包脚本
    ├── control-plane/               # 账号、角色、审核、反馈与审计后台
